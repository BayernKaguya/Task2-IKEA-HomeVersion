...
# 版本号：v3.4.2 Stable (实现2x2->4组合评估模式)
...
def h_calculator_three_dimensional(agg_data, operable_data, best_ld_shelf, coverage_target, allow_rotation, params, q):
    ...
    for i, (h1, h2) in enumerate(final_combinations):
        # Note: best_ld_shelf can be a single shelf dict or a tuple of two dicts
        ld_specs = best_ld_shelf if isinstance(best_ld_shelf, (list, tuple)) else (best_ld_shelf, best_ld_shelf)
        
        # New 2x2 -> 4 combination logic
        ...
        else: # Original logic for single L&D
            ld = ld_specs[0]
            shelves_to_evaluate = [
...
def get_detailed_unplaced_reasons_by_sku_id(unplaced_skus, final_shelves, h_max, allow_rotation):
    reasons = {}; representative_shelf = final_shelves[0]
    for _, sku in unplaced_skus.iterrows():
        sku_id = sku['sku_id']
        if sku['H'] > h_max: reasons[sku_id] = f"高度超限: SKU高({sku['H']:.0f}) > 最大允许高({h_max:.0f})"; continue
        ...
        for shelf in final_shelves:
            ...
            if can_fit_unrotated or can_fit_rotated: can_fit_any_shelf = True; break
        if not can_fit_any_shelf:
            ...
            elif sku['H'] > min_h_available: reasons[sku_id] = f"过高: SKU高({sku['H']:.0f}) > 货架净高"
            else: reasons[sku_id] = f"尺寸不匹配: L/D({sku['L']:.0f}x{sku['D']:.0f})与货架尺寸不符"
            continue
...
def calculate_ldh_utilization(final_solution, params):
    ...
    for i, shelf_spec in enumerate(final_shelves):
        ...
        total_available_length = (shelf_spec['Lp'] - 2 * SIDE_SPACING) * shelf_count # 可用长度
        l_util = total_placed_width / total_available_length if total_available_length > 0 else 0
        log_lines.append(f"  - L-利用率 (长度): {l_util:.2%}")
        
        # 2. D-利用率 (深度)
        total_depth_sum = 0; total_item_count = sum(item[2] for item in items_on_this_shelf)
        for item in items_on_this_shelf:
            sku, placed_width, count = item[0], item[1], item[2]
            # 确定摆放深度
            placed_depth = sku['D'] if placed_width == sku['L'] else sku['L']
            total_depth_sum += placed_depth * count
...
def calculation_worker(q, params, raw_data, shelves, agg_data=None):
    ...
    try:
        ...
        best_ld_for_h_calc, best_theoretical_ld, best_theoretical_coverage = (None, None, None)
        if params['ld_method'] == 'complementary':
        ...
        # 根据不同的H计算方法，使用正确的参数数量进行调用
        if h_method == 'manual':
            h_cand, eval_details = h_calculators[h_method](operable_data, h_max, params['p1'], params['p2'])
        elif h_method == 'three_dimensional':
            # three_dimensional 方法调用（7个参数）
        ...
        elif h_method == 'boundary':
            # boundary 方法调用（10个参数）
            if params['ld_method'] == 'complementary':
                # 在互补模式下，boundary方法缺少理论最优值，传入None
                main_ld_spec = best_ld_for_h_calc[0] # 该方法预期单个L&D规格作为基准
                h_cand, eval_details = h_calculators[h_method](agg_data, operable_data, main_ld_spec, h_max, params['coverage_target'], params['allow_rotation'], params, q, None, None)
            else: # ld_method is 'single'
                h_cand, eval_details = h_calculators[h_method](agg_data, operable_data, best_ld_for_h_calc, h_max, params['coverage_target'], params['allow_rotation'], params, q, best_theoretical_ld, best_theoretical_coverage)
        # ###################### 代码修改区域结束 ######################

        # 后续逻辑保持不变...
        feasible_solutions = [d for d in eval_details if d['is_feasible']] if h_method == 'three_dimensional' else []
        if feasible_solutions :
            best_solution_detail = max(feasible_solutions, key=lambda x: x['score'])
            final_solution = best_solution_detail['solution']
        elif h_method == 'three_dimensional' and eval_details:
             best_solution_detail = max(eval_details, key=lambda x: x['score'])
             final_solution = best_solution_detail['solution']
        else:
            ...
            if params['ld_method'] == 'complementary':
                ...
                final_shelves = [{'Lp': ld['Lp'], 'Dp': ld['Dp'], 'Wp': ld['Wp'], 'H': h1}, {'Lp': ld['Lp'], 'Dp': ld['Dp'], 'Wp': ld['Wp'], 'H': h2}]
            
            current_step = "步骤4: 最终分配与数量核算"; q.put(("log", f"\n--- {current_step} 开始 ---\n"))
        ...
        if final_solution['status'] == 'failure':
            s_info = ""
            for i, s in enumerate(final_solution['final_shelves']):
                s_info += f"  - 使用的货架规格 {i+1}: {s['Lp']:.0f}x{s['Dp']:.0f}x{s['H']:.0f}\n"
            raise ValueError(f"计算失败：在最终分配步骤中未能满足覆盖率目标 ({params['coverage_target']*100:.1f}%).\n\n"
                             f"--- 诊断信息 ---\n{s_info}"
                             f"  - 最终实现的SKU数量覆盖率: {final_solution['coverage_count']*100:.2f}%\n\n"
                             f"【建议】\n1. 尝试将覆盖率目标适度降低至 {final_solution['coverage_count']*100:.1f}% 或更低。\n2. 尝试使用不同的H计算方法。")
        ...
        q.put(("diagnostics", (unplaced_skus_df, detailed_reasons))); q.put(("visualization_data", (operable_data, eval_details, final_solution)))
        if params['sku_file']:
            success, path, error_msg = write_results_to_excel(params['sku_file'], placed_sku_ids, detailed_reasons, params['cols']['sku_id'], final_solution.get('sku_shelf_assignments'), final_shelves)
...
class App(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("货架配置优化工具 ProVersion-3.4.2 Stable")
        ...
        self.ld_method_var = tk.StringVar(value="complementary") # Default to complementary
        ...
        ctk.CTkLabel(self.frame_complementary_params, text="说明: 面积(差异性,越小越严), 尺寸(相似性,越大越严)", text_color="gray", font=ctk.CTkFont(size=10)).grid(row=2, column=0, columnspan=2, sticky="w")
        r+=1
    ...
    def select_sku_file(self):
        ...
        if file_path:
            self.update_path('sku', file_path)
    ...
    def select_shelf_file(self):
        ...
        if file_path:
            self.update_path('shelf', file_path)

    def paste_sku_path(self):
        try:
            path = self.clipboard_get().strip()
            if path and os.path.exists(path):
                self.update_path('sku', path)
            else:
                messagebox.showwarning("粘贴失败", "剪贴板中的路径无效或文件不存在")
        except tk.TclError:
            messagebox.showwarning("粘贴失败", "剪贴板为空或包含无效内容")

    def paste_shelf_path(self):
        try:
            path = self.clipboard_get().strip()
            if path and os.path.exists(path):
                self.update_path('shelf', path)
            else:
                messagebox.showwarning("粘贴失败", "剪贴板中的路径无效或文件不存在")
        except tk.TclError:
            messagebox.showwarning("粘贴失败", "剪贴板为空或包含无效内容")
    ...
    def display_welcome_message(self):
        self.update_textbox("""欢迎使用货架配置优化工具 ProVersion-3.4.2 Stable！

本工具旨在通过数据驱动的方式，为您推荐最优的货架规格组合。

v3.4.2 Stable 更新:
- 新增：当选用“两种货架(互补式)”算法时，程序将计算2组L&D与2组H，并自由组合成4种最终规格进行综合评估，以追求更优解。
- 优化：后台函数与结果展示部分已适配新的四规格模式。
- 修正：根据用户偏好更新了未安放原因的诊断文本。
...
""", True)
    ...
    def update_charts(self):
        try:
            ...
            fig = Figure(figsize=(10, 8), dpi=100); gs = fig.add_gridspec(2, 2, hspace=0.4, wspace=0.3)
            ax1 = fig.add_subplot(gs[0, 0]); ax2 = fig.add_subplot(gs[0, 1], polar=True)
            ax3 = fig.add_subplot(gs[1, 0]); ax4 = fig.add_subplot(gs[1, 1])
            operable_data, eval_details, final_solution = self.cache['viz_data']
            self.plot_heatmap(ax1, eval_details); self.plot_radar_chart(ax2, eval_details)
            self.plot_distribution_chart(ax3, operable_data, final_solution); self.plot_packing_demo(ax4, final_solution)
            self.canvas = FigureCanvasTkAgg(fig, master=self.charts_frame); self.canvas.draw()
            self.canvas.get_tk_widget().pack(side=tk.TOP, fill=tk.BOTH, expand=True)
        except Exception as e:
            self.charts_label.pack(expand=True); self.charts_label.configure(text=f"图表绘制失败: {e}")
            self.update_textbox(f"\n!!! 图表绘制失败: {e}!!!\n")
    def plot_heatmap(self, ax, eval_details):
        ax.set_title("综合评分热力图 (散点模拟)");
        if not eval_details: return
        df = pd.DataFrame(eval_details); feasible = df[df['is_feasible']]; infeasible = df[~df['is_feasible']]; best = feasible.nlargest(1, 'score')
        ax.scatter(infeasible['h1'], infeasible['h2'], c='lightgray', s=20, label='不可行解', alpha=0.6)
        ax.scatter(infeasible['h2'], infeasible['h1'], c='lightgray', s=20, alpha=0.6)
        if not feasible.empty:
            sc = ax.scatter(feasible['h1'], feasible['h2'], c=feasible['score'], cmap='coolwarm', s=50, label='可行解', alpha=0.8)
            ax.scatter(feasible['h2'], feasible['h1'], c=feasible['score'], cmap='coolwarm', s=50, alpha=0.8)
            plt.colorbar(sc, ax=ax, label='综合评分')
        if not best.empty:
            ax.scatter(best['h1'], best['h2'], c='red', marker='*', s=250, edgecolor='black', label='最优解', zorder=5)
            ax.scatter(best['h2'], best['h1'], c='red', marker='*', s=250, edgecolor='black', zorder=5)
        ax.set_xlabel("货架高度 H1 (mm)"); ax.set_ylabel("货架高度 H2 (mm)"); ax.grid(True, linestyle='--', alpha=0.5)
    def plot_radar_chart(self, ax, eval_details):
        ax.set_title("Top 5 方案指标对比雷达图")
        if not eval_details: return
        df = pd.DataFrame(eval_details); feasible = df[df['is_feasible']].copy()
        if feasible.empty: feasible = df.copy()
        top5 = feasible.nlargest(5, 'score')
        if top5.empty: return
        labels = ['数量利用率', '体积利用率', '高度利用率']; num_vars = len(labels)
        angles = np.linspace(0, 2 * np.pi, num_vars, endpoint=False).tolist(); angles += angles[:1]
        ax.set_theta_offset(np.pi / 2); ax.set_theta_direction(-1); ax.set_thetagrids(np.degrees(angles[:-1]), labels)
        for i, row in top5.iterrows():
            metrics = row['metrics']; values = [metrics['count_util'], metrics['volume_util'], metrics['height_util']]; values += values[:1]
            if i == top5.index[0]: ax.plot(angles, values, color='red', linewidth=2, linestyle='solid', label=f"最优解 (Score: {row['score']:.3f})"); ax.fill(angles, values, color='red', alpha=0.25)
            else: ax.plot(angles, values, linewidth=1, linestyle='dashed', label=f"方案 {i - top5.index[0] + 1} (Score: {row['score']:.3f})")
        ax.legend(loc='upper right', bbox_to_anchor=(1.35, 1.15), fontsize='small')
    def plot_distribution_chart(self, ax, operable_data, final_solution):
        ax.set_title("SKU高度分布与利用率曲线"); heights = operable_data['H']
        ax.hist(heights, bins=50, alpha=0.7, color='C0', label='SKU 高度分布')
        ax.set_xlabel("SKU 高度 (mm)"); ax.set_ylabel("SKU 数量", color='C0'); ax.tick_params(axis='y', labelcolor='C0')
        h_range = np.linspace(heights.min(), heights.max(), 100)
        count_util = [len(heights[heights <= h]) / len(heights) for h in h_range]
        volume_util = [(operable_data.loc[heights <= h, 'V'].sum()) / operable_data['V'].sum() for h in h_range]
        ax2 = ax.twinx()
        ax2.plot(h_range, count_util, color='C1', linestyle='--', label='数量利用率')
        ax2.plot(h_range, volume_util, color='C2', linestyle=':', label='体积利用率')
        ax2.set_ylabel("利用率", color='C1'); ax2.tick_params(axis='y', labelcolor='C1'); ax2.set_ylim(0, 1.05)
        
        unique_hs = sorted(list(set(s['H'] for s in final_solution['final_shelves'])))
        h1 = unique_hs[0]
        h2 = unique_hs[1] if len(unique_hs) > 1 else h1
        
        ax.axvline(x=h1, color='red', linestyle='-.', lw=2, label=f"最优高度1: {h1:.0f}mm")
        if h1 != h2:
            ax.axvline(x=h2, color='purple', linestyle='-.', lw=2, label=f"最优高度2: {h2:.0f}mm")
    ...
    def plot_packing_demo(self, ax, final_solution):
        ax.set_title("单层货架装箱效果示意图"); assignments = final_solution['assignments']
        if not assignments or all(not v for v in assignments.values()):
            ax.text(0.5, 0.5, "无SKU分配数据", ha='center', va='center')
            return
            
        # Find the shelf spec with the most SKU groups assigned to it for demonstration
        shelf_idx_to_demo = max(assignments, key=lambda k: len(assignments[k]))
        
        shelf_spec = final_solution['final_shelves'][shelf_idx_to_demo]; shelf_length = shelf_spec['Lp']
        items = assignments.get(shelf_idx_to_demo, [])
        if not items: return
        items_on_shelf = sorted([item for item in items for _ in range(item[2])], key=lambda x: x[1], reverse=True)
        ax.set_xlim(-0.05 * shelf_length, shelf_length * 1.05); ax.set_ylim(0, 1)
        ax.add_patch(patches.Rectangle((0, 0.2), shelf_length, 0.6, edgecolor='black', facecolor='lightgrey', lw=2))
        ax.text(shelf_length / 2, 0.9, f"货架L={shelf_length:.0f}mm", ha='center'); ax.set_xticks([]); ax.set_yticks([])
        current_pos = SIDE_SPACING; total_item_width = 0
        for item in items_on_shelf:
            width = item[1]
            if current_pos + width + (0 if current_pos == SIDE_SPACING else INTER_SPACING) <= shelf_length - SIDE_SPACING:
                if current_pos > SIDE_SPACING: current_pos += INTER_SPACING
                rect = patches.Rectangle((current_pos, 0.25), width, 0.5, facecolor=np.random.rand(3,), edgecolor='black')
                ax.add_patch(rect)
                ax.text(current_pos + width / 2, 0.5, f"{item[0]['L']:.0f}x{item[0]['D']:.0f}", ha='center', va='center', fontsize=6, color='white')
                current_pos += width; total_item_width += width
            else: break
        utilization = total_item_width / shelf_length if shelf_length > 0 else 0
        ax.set_xlabel(f"示意图 L-利用率 (仅SKU实体): {utilization:.1%}", fontsize=10)
    ...
    def display_diagnostics(self, unplaced_skus, detailed_reasons):
        ...
        for reason in detailed_reasons.values():
            ...
            if simple_reason == "托盘H不满足覆盖率要求": simple_reason = "优化策略：已满足覆盖率目标，未选择"
        ...
        self.update_textbox("\n" + "-" * 70 + "\n详细原因已写入到输出的Excel文件中。\n"); self.update_textbox("\n--- 改进建议 ---\n")
        operable_data = self.cache.get('operable_data', pd.DataFrame()); total_operable = len(operable_data)
        if total_operable > 0 and len(unplaced_skus) / total_operable > 0.2: self.update_textbox("未安放SKU占比较高，建议：\n1. 优先尝试降低覆盖率目标。\n2. 检查并增加尺寸更大或承重更高的候选货架规格。\n")
        else: self.update_textbox("未安放SKU占比较低，建议：\n1. 尝试微调覆盖率目标。\n2. 尝试不同的L&D或H计算算法组合，可能找到更优解。\n")
        self.update_textbox("="*70 + "\n")
