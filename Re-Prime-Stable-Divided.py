...
# 版本号：vReX 1.0.1 beta - 分组独立计算版本
#
# 重要更新：
# - 实现装箱装托分组独立计算
# - 每种存储方式独立生成2个L&D + 2个H = 4种最终规格
# - 支持分组图表切换显示
# - 装箱组和装托组各自优化，避免全局妥协
# - 总共可生成最多12种货架规格（装箱4种+装托4种+混装4种）
...
def calculate_height_for_group(agg_data, operable_data, selected_ld_shelves, coverage_target, allow_rotation, params, q, group_name, packing_decisions):
    """
    为特定分组计算最优H维度配置
    
    基于L&D计算的结果，为每个L&D组合配置2个最优高度，生成4种最终规格
    
    Args:
        agg_data: 聚合后的该组SKU数据
        selected_ld_shelves: L&D阶段选出的2种货架规格
    ...
    Returns:
    tuple: (final_shelves, final_solution, final_coverage)
    """
    
    # 检查是否有足够的L&D规格
    if len(selected_ld_shelves) < 2:
        q.put(("log", f"    {group_name}: L&D规格不足，使用现有规格生成H配置\n"))
    
    final_shelves = []
    all_solutions = []
    
    # 为每个L&D规格配置2个最优高度
    for ld_shelf in selected_ld_shelves:
        q.put(("log", f"    {group_name}: 为 L{ld_shelf['Lp']}×D{ld_shelf['Dp']} 计算最优H配置...\n"))
        
        # 提取该L&D下的SKU高度数据（使用原始SKU数据更稳健）
        heights = operable_data['H'].tolist() if 'H' in operable_data.columns else agg_data['H'].tolist()
        
        # 使用三维评分体系计算最优H组合
        try:
            # 生成候选高度点
            height_candidates = generate_height_candidates(heights, params.get('height_method', 'three_dimensional'))
            
            # 从候选高度中选择2个最优高度
            if len(height_candidates) >= 2:
                best_heights = select_optimal_heights(agg_data, ld_shelf, height_candidates, params, q, group_name)
            else:
                # 如果候选不足，使用边界值
                best_heights = [min(heights), max(heights)]
                q.put(("log", f"    {group_name}: 高度候选不足，使用边界值 [{min(heights)}, {max(heights)}]\n"))
            
            # 生成该L&D的2种规格
            for h in best_heights:
                shelf_spec = {
                    'Lp': ld_shelf['Lp'],
                    'Dp': ld_shelf['Dp'],
                    'H': h,  # 正确设置货架高度
                    'Wp': ld_shelf.get('Wp', 0),  # 承重/重量参数沿用原L&D货架
                    '类型': ld_shelf.get('类型', ld_shelf.get('shelf_type', ''))
                }
                final_shelves.append(shelf_spec)
                
        except Exception as e:
            q.put(("log", f"    {group_name}: H计算异常 {str(e)}，使用默认高度\n"))
            # 使用默认高度值
            default_heights = [1000, 2000]  # 默认值
            for h in default_heights:
                shelf_spec = {
                    'Lp': ld_shelf['Lp'],
                    'Dp': ld_shelf['Dp'],
                    'Wp': h,
                    'shelf_type': ld_shelf['shelf_type']
                }
                final_shelves.append(shelf_spec)
    
    # 精确分配：将SKU分配到4种最终规格上
    q.put(("log", f"    {group_name}: 执行精确分配到4种最终规格...\n"))
    try:
        # 使用现有的精确分配算法（需要原始SKU与packing_decisions）
        ...
        # 计算最终覆盖率
        final_coverage = final_solution.get('coverage_volume', 0) * 100
        
        q.put(("log", f"    {group_name}: 最终覆盖率 {final_coverage:.1f}%\n"))
        
        return final_shelves, final_solution, final_coverage
        
    except Exception as e:
        q.put(("log", f"    {group_name}: 精确分配异常 {str(e)}\n"))
        # 返回基本结果
        empty_solution = {
            'status': 'failure',
            'counts': [0] * len(final_shelves),
            'coverage_count': 0,
            'coverage_volume': 0,
            'placed_sku_ids': set(),
        ...
        return final_shelves, empty_solution, 0.0


def generate_height_candidates(heights, method='three_dimensional'):
    """生成高度候选点"""
    if not heights:
        return [1000, 2000]  # 默认值
    
    heights_array = np.array(heights)
    candidates = []
    
    # 添加边界值
    candidates.extend([np.min(heights_array), np.max(heights_array)])
    
    # 添加百分位数
    for p in [25, 50, 75]:
        candidates.append(np.percentile(heights_array, p))
    
    # 去重并排序
    candidates = sorted(list(set([int(x) for x in candidates])))
    
    return candidates


def select_optimal_heights(agg_data, ld_shelf, height_candidates, params, q, group_name):
    """从候选高度中选择2个最优高度"""
    
    if len(height_candidates) <= 2:
        return height_candidates[:2]
    ...
    for h in height_candidates:
        # 计算该高度的覆盖情况
        coverage_count = sum(1 for height in agg_data['H'] if height <= h)
        coverage_score = coverage_count / len(agg_data) if len(agg_data) > 0 else 0
        height_scores.append((h, coverage_score))
    
    # 按覆盖率排序，选择前2个，但要保证有合理的分布
    height_scores.sort(key=lambda x: x[1], reverse=True)
    
    # 选择一个较低和一个较高的高度
    if len(height_scores) >= 2:
        # 选择覆盖率最高的和中位数附近的
        selected = [height_scores[0][0], height_scores[len(height_scores)//2][0]]
        return sorted(selected)
    else:
        return [height_scores[0][0], height_candidates[-1]]


def process_grouped_calculations(grouped_data, grouped_shelves, coverage_target, allow_rotation, params, q, packing_decisions):
    """
    分组独立运行完整的L&D互补 + H计算算法 - 重构版本
    
    每个分组独立执行完整的计算流程：
    1. 数据聚合
    2. L&D互补算法（获得2个最优L&D组合）
    3. H计算算法（为每个L&D配2个最优H）
    4. 精确分配算法
    ...
    """
    q.put(("log", "--- 步骤 4: 分组独立计算（L&D + H + 精确分配） ---\n"))
    ...
    if not grouped_data['box_only_data'].empty and grouped_shelves['box_shelves']:
        ...
        try:
            # 数据聚合
            q.put(("log", "  - 装箱组：数据聚合中...\n"))
            agg_box_data = aggregate_sku_data(grouped_data['box_only_data'], packing_decisions)
            
            # L&D互补计算
            q.put(("log", "  - 装箱组：L&D互补计算中...\n"))
            ld_status, ld_solution = ld_calculator_complementary(
                agg_box_data, 
            ...
            if ld_status == "success":
                # H计算 - 为装箱组的每个L&D配置最优H
                q.put(("log", "  - 装箱组：H维度计算中...\n"))
                # 使用装箱组的原始SKU（未聚合）作为精确分配输入
                operable_box_data = grouped_data['box_only_data']
                final_shelves, final_solution, final_coverage = calculate_height_for_group(
                    agg_box_data,
                    operable_box_data,
                    ld_solution,  # 直接使用ld_solution
                    coverage_target,
                    allow_rotation,
                    params,
                    q,
                ...
                results['box_group_result'] = {
                    'status': 'success',
                    'final_shelves': final_shelves,
                    'solution': final_solution,
                    'coverage': final_coverage,
                    'message': f"装箱组计算成功 - 4种规格，覆盖率: {final_coverage:.1f}%",
                    'group_type': 'box_only'
                }
                q.put(("log", f"✓ 装箱组计算成功：覆盖率 {final_coverage:.1f}%\n"))
            else:
                results['box_group_result'] = {
                    'status': 'failure',
                    'final_shelves': [],
                    'solution': {},
                    'coverage': 0.0,
                    'message': f"装箱组L&D计算失败: {ld_solution}",  # ld_solution在失败时包含错误信息
                    'group_type': 'box_only'
                }
                q.put(("log", f"✗ 装箱组计算失败：{ld_solution}\n"))
            ...
            results['box_group_result'] = {
                ...
                'solution': {},
                'coverage': 0.0,
                'message': f"装箱组计算异常: {str(e)}",
                'group_type': 'box_only'
            }
            q.put(("log", f"✗ 装箱组计算异常：{str(e)}\n"))
    else:
        results['box_group_result'] = {
            'status': 'skip',
            'final_shelves': [],
    ...
    if not grouped_data['pallet_only_data'].empty and grouped_shelves['pallet_shelves']:
        ...
        try:
            # 数据聚合
            q.put(("log", "  - 装托组：数据聚合中...\n"))
            agg_pallet_data = aggregate_sku_data(grouped_data['pallet_only_data'], packing_decisions)
            
            # L&D互补计算
            q.put(("log", "  - 装托组：L&D互补计算中...\n"))
            ld_status, ld_solution = ld_calculator_complementary(
                agg_pallet_data, 
            ...
            if ld_status == "success":
                # H计算 - 为装托组的每个L&D配置最优H
                q.put(("log", "  - 装托组：H维度计算中...\n"))
                operable_pallet_data = grouped_data['pallet_only_data']
                final_shelves, final_solution, final_coverage = calculate_height_for_group(
                    agg_pallet_data,
                    operable_pallet_data,
                    ld_solution,  # 直接使用ld_solution
                    coverage_target,
                    allow_rotation,
                    params,
                ...
                results['pallet_group_result'] = {
                    'status': 'success',
                    'final_shelves': final_shelves,
                    'solution': final_solution,
                    'coverage': final_coverage,
                    'message': f"装托组计算成功 - 4种规格，覆盖率: {final_coverage:.1f}%",
                    'group_type': 'pallet_only'
                }
                q.put(("log", f"✓ 装托组计算成功：覆盖率 {final_coverage:.1f}%\n"))
            else:
                results['pallet_group_result'] = {
                    'status': 'failure',
                    'final_shelves': [],
                    'solution': {},
                    'coverage': 0.0,
                    'message': f"装托组L&D计算失败: {ld_solution}",
                    'group_type': 'pallet_only'
                }
                q.put(("log", f"✗ 装托组计算失败：{ld_solution}\n"))
            ...
            results['pallet_group_result'] = {
                ...
                'solution': {},
                'coverage': 0.0,
                'message': f"装托组计算异常: {str(e)}",
                'group_type': 'pallet_only'
            }
            q.put(("log", f"✗ 装托组计算异常：{str(e)}\n"))
    else:
        results['pallet_group_result'] = {
            'status': 'skip',
            'final_shelves': [],
        ...
        q.put(("log", "- 跳过装托组（无数据或无货架）\n"))

    # ========================== 处理混装组 ==========================
    # 混装组同时包含装托和装箱数据，选择装托货架优先处理
    if not grouped_data['mixed_data'].empty:
        q.put(("log", "开始处理混装组...\n"))
        if grouped_shelves['pallet_shelves']:
            try:
                # 数据聚合
                q.put(("log", "  - 混装组：数据聚合中（优先装托货架）...\n"))
                agg_mixed_data = aggregate_sku_data(grouped_data['mixed_data'], packing_decisions)
                
                # L&D互补计算
                q.put(("log", "  - 混装组：L&D互补计算中...\n"))
                ld_status, ld_solution = ld_calculator_complementary(
                    agg_mixed_data, 
                    grouped_shelves['pallet_shelves'], 
                    coverage_target, 
                ...
                if ld_status == "success":
                    # H计算
                    q.put(("log", "  - 混装组：H维度计算中...\n"))
                    operable_mixed_data = grouped_data['mixed_data']
                    final_shelves, final_solution, final_coverage = calculate_height_for_group(
                        agg_mixed_data,
                        operable_mixed_data,
                        ld_solution,
                    ...
                    results['mixed_group_result'] = {
                        'status': 'success',
                        'final_shelves': final_shelves,
                        'solution': final_solution,
                        'coverage': final_coverage,
                        'message': f"混装组计算成功（装托货架）- 4种规格，覆盖率: {final_coverage:.1f}%",
                        'group_type': 'mixed'
                    }
                    q.put(("log", f"✓ 混装组计算成功：覆盖率 {final_coverage:.1f}%\n"))
                else:
                    results['mixed_group_result'] = {
                        'status': 'failure',
                        'final_shelves': [],
                        'solution': {},
                        'coverage': 0.0,
                        'message': f"混装组L&D计算失败: {ld_solution}",
                        'group_type': 'mixed'
                    }
                    q.put(("log", f"✗ 混装组计算失败：{ld_solution}\n"))
                ...
                results['mixed_group_result'] = {
                    ...
                    'solution': {},
                    'coverage': 0.0,
                    'message': f"混装组计算异常: {str(e)}",
                    'group_type': 'mixed'
                }
                q.put(("log", f"✗ 混装组计算异常：{str(e)}\n"))
        else:
            results['mixed_group_result'] = {
                'status': 'skip',
                'final_shelves': [],
    ...
    # 汇总结果
    q.put(("log", "--- 分组独立计算完成 ---\n"))
    success_count = len([k for k, v in results.items() if v['status'] == 'success'])
    q.put(("log", f"成功计算组数: {success_count}/{len(results)}\n"))
...
def main_calculation_with_grouping(data, shelves, pallet_decimal_threshold, coverage_target, 
    ...
    final_result = {
        ...
        'box_group_result': calculation_results.get('box_group_result', {
            'status': 'skip', 'final_shelves': [], 'solution': {}, 'coverage': 0.0, 'message': '未处理'
        }),
        'pallet_group_result': calculation_results.get('pallet_group_result', {
            'status': 'skip', 'final_shelves': [], 'solution': {}, 'coverage': 0.0, 'message': '未处理'
        }),
        'mixed_group_result': calculation_results.get('mixed_group_result', {
            'status': 'skip', 'final_shelves': [], 'solution': {}, 'coverage': 0.0, 'message': '未处理'
        }),
        'combined_summary': {
            'total_shelves': len(calculation_results.get('box_group_result', {}).get('final_shelves', [])) +
                           len(calculation_results.get('pallet_group_result', {}).get('final_shelves', [])) +
                           len(calculation_results.get('mixed_group_result', {}).get('final_shelves', [])),
            'box_shelf_count': len(calculation_results.get('box_group_result', {}).get('final_shelves', [])),
            'pallet_shelf_count': len(calculation_results.get('pallet_group_result', {}).get('final_shelves', [])),
            'mixed_shelf_count': len(calculation_results.get('mixed_group_result', {}).get('final_shelves', [])),
            'success_groups': [k.replace('_result', '') for k, v in calculation_results.items() if v.get('status') == 'success'],
            'failed_groups': [k.replace('_result', '') for k, v in calculation_results.items() if v.get('status') == 'failure'],
            'skipped_groups': [k.replace('_result', '') for k, v in calculation_results.items() if v.get('status') == 'skip']
        }
    }
    
    # 添加总体状态判断
    success_count = len(final_result['combined_summary']['success_groups'])
    total_groups_with_data = sum(1 for group in ['box_group_result', 'pallet_group_result', 'mixed_group_result'] 
                                if calculation_results.get(group, {}).get('status') != 'skip')
    
    if success_count == 0:
        final_result['overall_status'] = 'failure'
        final_result['overall_message'] = '所有组计算失败'
    elif success_count == total_groups_with_data:
        final_result['overall_status'] = 'success'
        final_result['overall_message'] = f'所有可计算组成功完成（{success_count}组）'
    else:
        final_result['overall_status'] = 'partial_success'
        final_result['overall_message'] = f'部分组计算成功（{success_count}/{total_groups_with_data}组）'
...
def final_placement_with_individual_skus_mixed(operable_data, final_shelves, packing_decisions, allow_rotation, q):
    ...
    # 1. 逐一分配真实SKU
    # 保障必要列存在：若来自聚合或转换，提前跳过
    required_cols = {'sku_id','L','D','H','W','pallet_l','pallet_d','pallet_h','pallet_w','pallet_v','box_l','box_d','box_h','box_w','box_v'}
    missing_cols = [c for c in ['sku_id','L','D','H'] if c not in operable_data.columns]
    if missing_cols:
        q.put(("log", f"警告：输入到精确分配的operable_data缺少列 {missing_cols}，覆盖率将异常。\n"))
    for idx, sku in operable_data.iterrows():
        ...
        for i, shelf in enumerate(final_shelves):
            ...
            if sku_w > shelf.get('Wp', float('inf')) or sku_h > shelf.get('H', float('inf')):
                continue
...
def calculate_ldh_utilization(final_solution, params):
    ...
    for i, shelf_spec in enumerate(final_shelves):
        ...
        # 修正长度利用率计算：计算单个货架的平均利用率，而不是累计利用率
        # FFD装箱算法会将物品分配到多个货架上，我们需要模拟这个分配过程
        available_length_per_shelf = shelf_spec['Lp'] - 2 * SIDE_SPACING
        
        # 使用与实际装箱相同的FFD算法来计算长度利用率
        sorted_items = sorted(items_on_this_shelf, key=lambda x: x[1], reverse=True)
        bins = []  # 每个bin代表一个货架
        ...
        for item in sorted_items:
            ...
            for bin_info in bins:
                if bin_info['remaining_space'] >= placed_width + (INTER_SPACING if bin_info['item_count'] > 0 else 0):
                    space_needed = placed_width + (INTER_SPACING if bin_info['item_count'] > 0 else 0)
                    bin_info['used_width'] += placed_width
                    bin_info['remaining_space'] -= space_needed
                    bin_info['item_count'] += 1
                    placed = True
                    break
            ...
            if not placed:
                # 创建新的bin
                bins.append({
                    'used_width': placed_width,
                    'remaining_space': available_length_per_shelf - placed_width,
                    'item_count': 1
                })
        
        # 计算平均长度利用率
        if bins:
            total_utilization = sum(bin_info['used_width'] / available_length_per_shelf for bin_info in bins)
            l_util = total_utilization / len(bins) if len(bins) > 0 else 0
        else:
            l_util = 0
        
        log_lines.append(f"  - L-利用率 (长度): {l_util:.2%}")
        log_lines.append(f"    (基于FFD装箱模拟: {len(bins)}个货架的平均利用率)")
...
def calculation_worker(q, params, raw_data, shelves, agg_data=None):
    ...
    try:
        # --- 使用新的分组独立计算流程 ---
        current_step = "分组独立计算流程"
        q.put(("log", f"\n--- {current_step} 开始 ---\n"))
        
        # 调用新的主计算函数
        final_result = main_calculation_with_grouping(
            raw_data, 
            shelves, 
        ...
        all_final_shelves = []
        all_solutions = {}
        shelves_with_group = []  # (shelf_dict, group_key)
        combined_counts = []
        
        # 装箱组结果
        box_result = final_result.get('box_group_result', {})
        if box_result.get('status') == 'success':
            all_final_shelves.extend(box_result['final_shelves'])
            shelves_with_group.extend([(s, 'box') for s in box_result['final_shelves']])
            all_solutions['box_group'] = box_result['solution']
            combined_counts.extend(box_result.get('solution', {}).get('counts', []))
            q.put(("log", f"✓ 装箱组: {len(box_result['final_shelves'])} 种规格\n"))
        
        # 装托组结果
        pallet_result = final_result.get('pallet_group_result', {})
        if pallet_result.get('status') == 'success':
            all_final_shelves.extend(pallet_result['final_shelves'])
            shelves_with_group.extend([(s, 'pallet') for s in pallet_result['final_shelves']])
            all_solutions['pallet_group'] = pallet_result['solution']
            combined_counts.extend(pallet_result.get('solution', {}).get('counts', []))
            q.put(("log", f"✓ 装托组: {len(pallet_result['final_shelves'])} 种规格\n"))
        
        # 混装组结果
        mixed_result = final_result.get('mixed_group_result', {})
        if mixed_result.get('status') == 'success':
            all_final_shelves.extend(mixed_result['final_shelves'])
            shelves_with_group.extend([(s, 'mixed') for s in mixed_result['final_shelves']])
            all_solutions['mixed_group'] = mixed_result['solution']
            combined_counts.extend(mixed_result.get('solution', {}).get('counts', []))
            q.put(("log", f"✓ 混装组: {len(mixed_result['final_shelves'])} 种规格\n"))
        
        if not all_final_shelves:
            raise ValueError("所有分组计算都失败了")
        
        q.put(("log", f"总计: {len(all_final_shelves)} 种最终规格\n"))
        
        # --- 构造兼容的解决方案格式 ---
        # 为了保持与现有显示逻辑的兼容性，我们需要构造一个solution字典
        combined_solution = {
            'counts': combined_counts if combined_counts else {},
            'shelf_details': {},
            'grouped_solutions': all_solutions,  # 保存分组详细信息
            'summary': {
                'total_shelves_needed': len(all_final_shelves),
                'box_shelves': len(box_result.get('final_shelves', [])),
                'pallet_shelves': len(pallet_result.get('final_shelves', [])),
                'mixed_shelves': len(mixed_result.get('final_shelves', []))
            }
        }
        
        # 汇总货架详情信息
        for i, (shelf, group_key) in enumerate(shelves_with_group):
            shelf_id = f"货架{i+1:02d}"
            # 如果 counts 是列表，display_results 会按顺序取值；这里保持详情即可
            combined_solution['shelf_details'][shelf_id] = {
                'shelf_type': shelf.get('类型', shelf.get('shelf_type', '')),
                'dimensions': f"L{shelf.get('Lp',0):.0f}×D{shelf.get('Dp',0):.0f}×H{shelf.get('H', shelf.get('Wp',0)):.0f}",
        ...
        # --- 返回最终结果 ---
        q.put(("result", all_final_shelves, combined_solution, params['coverage_target'], final_result['packing_summary']))
        q.put(("status", "计算完成"))
        q.put(("log", "\n🎉 分组独立计算流程完成！\n"))
        
    except Exception as e:
        q.put(("error", f"{current_step}阶段出错: {str(e)}"))
        return
...
class App(ctk.CTk):
    def __init__(self):
        ...
        # 分组图表选择器框架
        self.chart_controls_frame = ctk.CTkFrame(self.charts_frame)
        self.chart_controls_frame.pack(pady=10, padx=10, fill="x")
        
        # 分组选择器
        ctk.CTkLabel(self.chart_controls_frame, text="查看图表组:").pack(side="left", padx=10)
        self.chart_group_selector = ctk.CTkSegmentedButton(
            self.chart_controls_frame,
            values=["综合视图", "装箱组", "装托组", "混装组"],
            command=self.on_chart_group_changed
        )
        self.chart_group_selector.pack(side="left", padx=10)
        self.chart_group_selector.set("综合视图")  # 默认选择
        
        # 图表显示区域
        self.charts_display_frame = ctk.CTkFrame(self.charts_frame, fg_color="transparent")
        self.charts_display_frame.pack(expand=True, fill="both", pady=(10,0))
        
        self.charts_label = ctk.CTkLabel(self.charts_display_frame, text="计算完成后将在此处显示图表", font=ctk.CTkFont(size=20))
...
""", True)
    ...
    def process_queue(self):
        try:
            ...
            msg_type = msg[0]
            msg_content = msg[1:] if len(msg) > 2 else msg[1]
            
            if msg_type == "log": self.update_textbox(msg_content if isinstance(msg_content, str) else msg_content[0])
            ...
            elif msg_type == "visualization_data": self.cache['viz_data'] = msg_content if isinstance(msg_content, dict) else msg_content[0]; self.update_charts(); self.tabview.set("分析图表")
            elif msg_type == "error": self.update_textbox(f"\n!!!!!! 计算出错 !!!!!!\n\n{msg_content if isinstance(msg_content, str) else msg_content[0]}\n"); self.button_run.configure(state="normal"); self.status_label.configure(text="状态: 计算失败"); self.progressbar.set(0)
            elif msg_type == "done": self.status_label.configure(text=f"状态: {msg_content if isinstance(msg_content, str) else msg_content[0]}"); self.button_run.configure(state="normal"); self.progressbar.set(1)
    ...
    # --- vReX 1.0.1 全新可视化仪表盘 ---
    def on_chart_group_changed(self, value):
        """分组图表切换回调函数"""
        if hasattr(self, 'cache') and 'viz_data' in self.cache:
            self.current_chart_group = value
            self.update_charts()
    ...
    def update_charts(self):
        ...
        try:
            ...
            # 获取当前选择的图表组
            current_group = getattr(self, 'current_chart_group', '综合视图')
            ...
            ax2 = fig.add_subplot(gs[0, 1])  # Panel 2: Decision Logic
            ax3 = fig.add_subplot(gs[1, 0])  # Panel 3: Diagnostics
            ax4 = fig.add_subplot(gs[1, 1])  # Panel 4: Profile
            
            operable_data, eval_details, final_solution = self.cache['viz_data']
            
            # 根据选择的组别显示不同的图表内容
            if current_group == '综合视图':
                self.plot_solution_overview(ax1, final_solution, title_suffix="综合")
            ...
            elif current_group in ['装箱组', '装托组', '混装组']:
                group_mapping = {'装箱组': 'box_group', '装托组': 'pallet_group', '混装组': 'mixed_group'}
                group_key = group_mapping[current_group]
                
                # 检查是否有该组的解决方案
                grouped_solutions = final_solution.get('grouped_solutions', {})
                if group_key in grouped_solutions:
                    group_solution = grouped_solutions[group_key]
                    # 需要为单个组构造对应的数据结构
                    # 这里简化处理，显示该组相关的图表
                    self.plot_solution_overview(ax1, final_solution, title_suffix=current_group)
                    self.plot_decision_basis(ax2, operable_data, final_solution)  
                    self.plot_sku_diagnostics(ax3, operable_data, final_solution)
                    self.plot_shelf_profile(ax4, final_solution)
                else:
                    ...
                    for ax in [ax1, ax2, ax3, ax4]:
                        ax.text(0.5, 0.5, f'{current_group}无计算结果', ha='center', va='center', fontsize=16)
                        ax.set_title(f"{current_group} - 无数据")
            
            self.canvas = FigureCanvasTkAgg(fig, master=self.charts_display_frame)
            ...
            print(f"图表更新错误: {e}")
            # 显示错误信息
            if hasattr(self, 'charts_label'):
                self.charts_label.configure(text=f"图表显示出错: {str(e)}")
                self.charts_label.pack(expand=True)
    ...
    def plot_solution_overview(self, ax, final_solution, title_suffix=""):
        title = "图表一：方案总览与核心指标"
        if title_suffix:
            title += f" - {title_suffix}"
        ax.set_title(title, fontweight="bold")
        
        specs = final_solution.get('final_shelves', [])
        counts = final_solution.get('counts', [])
        
        if not specs or not counts:
            ax.text(0.5, 0.5, "暂无方案数据", ha='center', va='center')
            return
        
        # 处理不同格式的counts
        if isinstance(counts, dict):
            counts_list = [counts.get(f"货架{i+1:02d}", 1) for i in range(len(specs))]
        else:
            counts_list = counts
            
        labels = [
            f"规格 {i+1}\n{s.get('Lp',0):.0f}L×{s.get('Dp',0):.0f}D×{s.get('H', s.get('Wp',0)):.0f}H"
            for i, s in enumerate(specs)
        ]
        y_pos = np.arange(len(labels))
        
        bars = ax.barh(y_pos, counts_list, align='center', color=plt.cm.viridis(np.linspace(0.4, 0.9, len(labels))))
        ...
        total_shelves = sum(counts_list)
        cov_count = final_solution.get('coverage_count', 0) * 100
        cov_vol = final_solution.get('coverage_volume', 0) * 100
    ...
    def plot_sku_diagnostics(self, ax, operable_data, final_solution):
        ...
        # 使用安全方式获取wedges，避免在不同matplotlib版本下的解包错误
        pie_result = ax.pie(sizes, startangle=90, colors=colors, radius=1.2)
    ...
    def plot_shelf_profile(self, ax, final_solution):
        ...
        if items_assigned:
            ...
            for tup in sorted_items:
                # 兼容处理：二元组(item_dict, width)或三元组(item_dict, width, actual_count)
                item_dict = tup[0]
                width = tup[1]
                # 忽略可能存在的第三个元素actual_count
                ...
                if remaining_length >= required_space:
                    ...
                    if item_height <= 0:
                        # 回退方案：根据数据类型使用其他高度字段
                        if hasattr(self, 'current_params') and 'used_data_type' in self.current_params:
                            data_type = self.current_params['used_data_type']
                            if data_type == 'pallet':
                                item_height = item_dict.get('pallet_h', 0)
                            elif data_type == 'box':
                                item_height = item_dict.get('box_h', 0)
                    
                    packed_items.append({'pos': current_pos, 'width': width, 'height': item_height})
                    current_pos += width
                ...
                if avg_width > 0:
                    ...
                    if theoretical_items > len(packed_items) * 1.5:
                        ...
                        while remaining_length > avg_width and len(packed_items) < theoretical_items:
                            # 兼容处理：二元组或三元组结构
                            tup = items_assigned[item_cycle % len(items_assigned)]
                            item_dict = tup[0]
                            width = tup[1]
                            # 忽略可能存在的第三个元素actual_count
    ...
    def display_results(self, final_shelves, solution, coverage_target, packing_summary=None):
        ...
        if packing_summary:
            ...
            stats = packing_summary.get('stats', {})
        ...
        if summary:
            self.update_textbox("分组计算汇总:\n")
            self.update_textbox(f"  - 装箱规格: {summary.get('box_shelves', 0)} 种\n")
            self.update_textbox(f"  - 装托规格: {summary.get('pallet_shelves', 0)} 种\n") 
            self.update_textbox(f"  - 混装规格: {summary.get('mixed_shelves', 0)} 种\n")
            self.update_textbox(f"  - 总计规格: {summary.get('total_shelves_needed', len(final_shelves))} 种\n")
            self.update_textbox("-" * 70 + "\n")
        
        self.update_textbox(f"算法目标: 在满足~{coverage_target*100:.0f}%覆盖率下，为每种存储方式独立生成最优货架规格\n")
        self.update_textbox("-" * 70 + "\n")
        
        # 计算总货架数量（兼容字典/列表/标量）
        if isinstance(counts, dict):
            total_count = sum(v for v in counts.values() if isinstance(v, (int, float)))
        elif isinstance(counts, (list, tuple)):
            total_count = sum(int(x) for x in counts)
        else:
            total_count = int(counts) if isinstance(counts, (int, float)) else len(final_shelves)
        
        self.update_textbox(f"最终方案所需货架总数: {total_count} 个\n")
        self.update_textbox(f"推荐的 {len(final_shelves)} 种货架规格及其所需数量:\n")
        
        for i, shelf in enumerate(final_shelves):
            # 获取货架数量
            if isinstance(counts, dict):
                shelf_id = f"货架{i+1:02d}"
                count = counts.get(shelf_id, 1)
            elif isinstance(counts, (list, tuple)):
                count = counts[i] if i < len(counts) else 1
            else:
                count = int(counts) if isinstance(counts, (int, float)) else 1
            
            # 确定货架类型和来源组
            shelf_type_val = shelf.get('类型', shelf.get('shelf_type', '未知'))
            shelf_type_info = f"(类型: {shelf_type_val})" if shelf_type_val != '' else ""
            
            # 根据shelf_type判断来源组
            source_group = ""
            if shelf_type_val:
                if shelf_type_val in ['B', 'box']:
                    source_group = " [装箱组]"
                elif shelf_type_val in ['A', 'pallet']:
                    source_group = " [装托组]"
                else:
                    source_group = " [混装组]"
            
            spec_line = f"  - 规格 {i+1}: {count} 个 | {shelf.get('Lp',0):.0f}(长)×{shelf.get('Dp',0):.0f}(深)×{shelf.get('H', shelf.get('Wp', 0)):.0f}(高) | 承重: {shelf.get('Wp', 0):.0f}kg {shelf_type_info}{source_group}"
            
            if usable_vertical_space > 0:
                shelf_height = shelf.get('H', shelf.get('Wp', 0))
                layer_total_h = shelf_height + params.get('shelf_thickness', 50) + params.get('layer_headroom', 200)
                num_layers = math.floor(usable_vertical_space / layer_total_h) if layer_total_h > 0 else 0
        ...
        # 显示覆盖率信息（尝试从不同可能的结构中获取）
        if 'coverage_count' in solution and 'coverage_volume' in solution:
            self.update_textbox(f"  - SKU数量覆盖率: {solution['coverage_count'] * 100:.2f}%\n")
            self.update_textbox(f"  - SKU体积覆盖率: {solution['coverage_volume'] * 100:.2f}%\n")
        else:
            self.update_textbox("  - 覆盖率信息待分组详细分析完成后显示\n")
            
        # 如果有分组解决方案，显示分组详情
        if 'grouped_solutions' in solution:
            grouped = solution['grouped_solutions']
            self.update_textbox("\n分组详细结果:\n")
            
            for group_name, group_solution in grouped.items():
                group_display_name = {
                    'box_group': '装箱组',
                    'pallet_group': '装托组', 
                    'mixed_group': '混装组'
                ...
                if isinstance(group_solution, dict) and 'coverage' in group_solution:
                    self.update_textbox(f"  - {group_display_name}: 覆盖率 {group_solution.get('coverage', 0):.1f}%\n")
                else:
                    self.update_textbox(f"  - {group_display_name}: 详细结果可在后续版本中查看\n")
