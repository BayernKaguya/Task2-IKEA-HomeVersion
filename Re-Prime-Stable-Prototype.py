...
def calculate_ldh_utilization(final_solution, params):
    ...
    for i, shelf_spec in enumerate(final_shelves):
        ...
        # vReX 1.0.1 修正: 正确处理不同的数据结构
        # 检查数据结构：三元组 (sku_dict, placed_width, actual_count) 或二元组 (sku_dict, placed_width)
        if len(items_on_this_shelf[0]) == 3:
            # 三元组结构：考虑实际业务数量
            total_placed_width = sum(item[1] * item[2] for item in items_on_this_shelf)  # 宽度×数量
            total_business_count = sum(item[2] for item in items_on_this_shelf)  # 实际业务数量
            total_item_count = len(items_on_this_shelf)  # SKU种类数
        else:
            # 二元组结构：向后兼容
            total_placed_width = sum(item[1] for item in items_on_this_shelf)
            total_business_count = len(items_on_this_shelf)
            total_item_count = len(items_on_this_shelf)
        
        # 修正深度利用率计算：根据实际放置情况和业务数量计算使用的深度
        total_depth_sum = 0
        total_sku_h_sum = 0
        ...
        for item in items_on_this_shelf:
            ...
            placed_width = item[1]
            # 获取实际数量
            actual_count = item[2] if len(item) == 3 else 1
            ...
            # 累计深度和高度，考虑实际数量
            total_depth_sum += actual_depth * actual_count
            total_sku_h_sum += sku_dict['H'] * actual_count

        # L利用率 = 实际放置的总宽度 / 货架总可用长度
        total_available_length = shelf_spec['Lp'] * shelf_count
        l_util = total_placed_width / total_available_length if total_available_length > 0 else 0
        log_lines.append(f"  - L-利用率 (长度): {l_util:.2%}")
        
        # D利用率 = 加权平均深度 / 货架深度
        weighted_avg_depth = total_depth_sum / total_business_count if total_business_count > 0 else 0
        d_util = weighted_avg_depth / shelf_spec['Dp'] if shelf_spec['Dp'] > 0 else 0
        log_lines.append(f"  - D-利用率 (深度): {d_util:.2%}")

        # H利用率 = 加权平均高度 / 货架高度
        avg_sku_h = total_sku_h_sum / total_business_count if total_business_count > 0 else 0
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.tabview.add("运行日志"); self.tabview.set("运行日志")
