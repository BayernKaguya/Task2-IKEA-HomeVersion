...
def maximize_placement_without_coverage_limit(agg_data, final_shelves, allow_rotation):
    """
    基于最优货架标准进行最大化填充，忽略覆盖率限制。
    只要SKU在物理上能放入货架，就必须放入。
    """
    ...
    for _, sku_group in agg_data.iterrows():
        ...
        for i, shelf in enumerate(final_shelves):
            # 物理限制检查：重量、高度
            if sku_group['W'] > shelf['Wp'] or sku_group['H'] > shelf['H']:
                continue
        ...
        if possible_fits:
            ...
            # 记录每个SKU的货架分配
            for sku_id in sku_group['sku_ids']:
                sku_shelf_assignments[sku_id] = best_fit['shelf_idx']
    
    # 计算实际覆盖率（用于报告，不用于判断成败）
    ...
    return {
        'status': 'success',  # 最大化填充总是成功的，只要有货架规格
        'counts': final_counts,
    ...
    for _, sku in unplaced_skus.iterrows():
        ...
        if not can_fit_any_shelf:
            ...
            if sku['W'] > representative_shelf['Wp']:
              ...
              # v3.5.1: 逻辑上这个分支不应该被触发，因为所有能放的都放了
              # 但为防止意外，保留一个通用原因
...
def calculation_worker(q, params, raw_data, shelves, agg_data=None):
    ...
    try:
        ...
        if optimal_shelves is None:
            raise ValueError("无法确定最终的货架规格，请检查前面步骤的计算结果。")
        
        # 使用1.0的覆盖率目标进行全力填充
        final_solution = final_allocation_and_counting(agg_data, optimal_shelves, 1.0, params['allow_rotation'])
        q.put(("log", "最大化填充计算完成。\n"))

        if final_solution['status'] == 'failure' and not final_solution['placed_sku_ids']:
...
class App(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("货架配置优化工具 ProVersion-3.5.4 beta")
    ...
    def display_welcome_message(self):
        self.update_textbox("""欢迎使用货架配置优化工具 ProVersion-3.5.4 beta！

v3.5.1 beta 更新:
- 核心逻辑升级为“标准确定，全力填充”模式，优先确定最优货架规格，再最大化安放所有可容纳的SKU。
- 移除了“因满足覆盖率未选择”的诊断原因，未安放原因现在仅为物理限制。
...
""", True)
    ...
    def plot_distribution_chart(self, ax, operable_data, final_solution):
        ax.set_title("SKU高度分布与最优高度选择"); heights = operable_data['H']
        ...
        ax2.plot(h_range, count_util, color='C1', linestyle='--', label='数量覆盖率曲线')
        ax2.set_ylabel("覆盖率", color='C1'); ax2.tick_params(axis='y', labelcolor='C1'); ax2.set_ylim(0, 1.05)
    ...
    def plot_packing_demo(self, ax, final_solution):
        ...
        if not assignments or all(not v for v in assignments.values()):
            ax.text(0.5, 0.5, "无SKU分配数据", ha='center', va='center'); return
    ...
    def display_diagnostics(self, unplaced_skus, detailed_reasons):
        ...
        self.update_textbox(f"总共有 {len(unplaced_skus)} 个可操作SKU因物理限制未能安放，原因汇总如下:\n")
